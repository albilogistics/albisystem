...
app.post('/api/sync/manual', async (req, res) => {
  try {
    ...
    // For now, return success without scraping (puppeteer removed)
    res.json({
      success: true,
      message: 'Sync endpoint available (scraping disabled for deployment)',
      data: []
    });
  } catch (error) {
...
app.get('/api/costs', async (req, res) => {
  try {
    ...
    // Calculate totals
    const totalCost = productsWithCustomerPrices.reduce((sum, product) => sum + product.totalCost, 0);
    const totalSellPrice = productsWithCustomerPrices.reduce((sum, product) => sum + product.sellPrice, 0);
    const totalCommission = productsWithCustomerPrices.reduce((sum, product) => sum + product.commission, 0);
    const totalCustomerPrice = productsWithCustomerPrices.reduce((sum, product) => sum + product.customerPrice, 0);
    
    console.log(`  totalCost: ${totalCost}, sellPrice: ${totalSellPrice}, commission: ${totalCommission}, customerPrice: ${totalCustomerPrice}`);
    ...
    res.json({
      ...
      summary: {
        ...
        totalCommission,
        totalCustomerPrice,
        totalProducts: productsWithCustomerPrices.length
      }
    });
  } catch (error) {
